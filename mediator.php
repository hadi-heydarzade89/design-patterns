...
class EventDispatcher
{
    ...
    private function getEventObserver(string $event = "*"): array
    {
    ...
    {
        foreach ($this->getEventObserver($event) as $key => $s) {
            if ($s === $observer) {
...
class UserRepository implements Observer
{
    ...
    public function createUser(array $data, bool $silent = false): User
    {
        ...
        $this->users[$id] = $user;
        if (!$silent) {
            events()->trigger("user:created", $this, $user);
        }
        return $user;
    }
    ...
    public function updateUser(User $user, array $data, bool $silent = false): ?User
    {
        ...
        $user->update($data);
        if (!$silent) {
            events()->trigger("user:updated", $this, $user);
        }
        return $user;
    }
    ...
    public function deleteUser(User $user, bool $silent = false): void
    {
        ...
        if (!$silent) {
            events()->trigger("user:deleted", $this, $user);
        }
    }
}
...
class User
{
    ...
    public function delete(): void
    {
        ...
        events()->trigger("user:deleted", $this, $this);
    }
}
